...
class SON(dict):
    ...
    .. [#re] `~bson.regex.Regex` instances and regular expression
       objects from ``re.compile()`` are both saved as BSON regular expressions.
       BSON regular expressions are decoded as `~bson.regex.Regex`
       instances.
    .. [#bytes] The bytes type from Python 3.x is encoded as BSON binary with
       ...
       it will be decoded to an instance of `~bson.binary.Binary` with
       subtype 0.
    """
