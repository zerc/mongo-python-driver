...
`~.mongo_client.MongoClient`, or you could simply call
...
`~mongoclient.MongoClient` Changes
..................................

`~mongo_client.MongoClient` is now the one and only
...
`~mongo_client.MongoClient` now also supports the full
`~read_preferences.ReadPreference` API.

The obsolete ``MasterSlaveConnection`` class is removed.

The `~mongo_client.MongoClient` constructor no
longer blocks while connecting to the server or servers, and it no
longer raises `~errors.ConnectionFailure` if they
are unavailable, nor `~errors.ConfigurationError`
...
In PyMongo 2.x, `~mongo_client.MongoClient` accepted a list of standalone
...
The ``conn_id`` property is renamed to `~cursor.Cursor.address`.

Cursor management changes
.........................

`~cursor_manager.CursorManager`  and
`~.MongoClient.set_cursor_manager` are no longer deprecated.
If you subclass `~cursor_manager.CursorManager` your
implementation of `~cursor_manager.CursorManager.close` must now
take a second parameter, ``address``. The ``BatchCursorManager`` class is
removed.

The second parameter to `~.MongoClient.close_cursor` is renamed
from ``_conn_id`` to ``address``. `~.MongoClient.kill_cursors`
...
Since PyMongo 1.6, methods ``open`` and ``close`` of `~gridfs.GridFS`
...
is removed from `~collection.Collection.find`,
`~collection.Collection.find_one`,
`~collection.Collection.aggregate`,
`~database.Database.command`, and so on.
PyMongo now always represents BSON regular expressions as
`~bson.regex.Regex` objects. This prevents errors for incompatible
patterns, see `PYTHON-500`_. Use `~bson.regex.Regex.try_compile` to
...
`~bson.codec_options.CodecOptions` to configure these options. The
APIs affected are:
...
- Support passing an index name to `~cursor.Cursor.hint`.
- The `~cursor.Cursor.count` method will use a hint if one
  has been provided through `~cursor.Cursor.hint`.
...
.. note:: There are a number of deprecations in this release for features that
  ...
  - `~mongo_client.MongoClient.start_request`
  - `~mongo_client.MongoClient.in_request`
  - `~mongo_client.MongoClient.end_request`
  - `~mongo_client.MongoClient.copy_database`
  - `~database.Database.error`
  - `~database.Database.last_status`
  - `~database.Database.previous_error`
  - `~database.Database.reset_error_history`
...
- Support for server side query timeouts using
  `~cursor.Cursor.max_time_ms`.
- Support for writing `~collection.Collection.aggregate`
  output to a collection.
- A new `~collection.Collection.parallel_scan` helper.
- `~errors.OperationFailure` and its subclasses now include
  a `~errors.OperationFailure.details` attribute with complete
...
the `~collection.Collection.insert` method.
...
- The ``max_pool_size`` option for `~mongo_client.MongoClient`
  and `~mongo_replica_set_client.MongoReplicaSetClient` now
  actually caps the number of sockets the pool will open concurrently.
  Once the pool has reached `~mongo_client.MongoClient.max_pool_size`
  ...
  will raise `~errors.ConnectionFailure` after the timeout. By
  default ``waitQueueTimeoutMS`` is not set.
  See :ref:`connection-pooling` for more information.
- The `~collection.Collection.insert` method automatically splits
  large batches of documents into multiple insert messages based on
  `~mongo_client.MongoClient.max_message_size`
- Support for the exhaust cursor flag.
  See `~collection.Collection.find` for details and caveats.
- Support for the PLAIN and MONGODB-X509 authentication mechanisms.
  ...
  information.
- Support aggregation output as a `~cursor.Cursor`. See
  `~collection.Collection.aggregate` for details.
...
.. note:: `~database.Database.authenticate` now raises a
    subclass of `~errors.PyMongoError` if authentication
...
`~collection.Collection.aggregate`, and possibly other commands,
with mongos.
...
- New `~mongo_client.MongoClient` and
  `~mongo_replica_set_client.MongoReplicaSetClient` classes -
  these connection classes do acknowledged write operations (previously referred
  to as 'safe' writes) by default. `~connection.Connection` and
  `~replica_set_connection.ReplicaSetConnection` are deprecated
  but still support the old default fire-and-forget behavior.
- A new write concern API implemented as a
  `~collection.Collection.write_concern` attribute on the connection,
  `~database.Database`, or `~collection.Collection`
  classes.
- `~mongo_client.MongoClient` (and `~connection.Connection`)
  now support Unix Domain Sockets.
- `~cursor.Cursor` can be copied with functions from the `copy`
  module.
- The `~database.Database.set_profiling_level` method now supports
  a ``slow_ms`` option.
- The replica set monitor task (used by
  `~mongo_replica_set_client.MongoReplicaSetClient` and
  `~replica_set_connection.ReplicaSetConnection`) is a daemon thread
  once again, meaning you won't have to call
  `~mongo_replica_set_client.MongoReplicaSetClient.close` before
  exiting the python interactive shell.

.. warning::

    The constructors for `~mongo_client.MongoClient`,
    `~mongo_replica_set_client.MongoReplicaSetClient`,
    `~connection.Connection`, and
    `~replica_set_connection.ReplicaSetConnection` now raise
    `~errors.ConnectionFailure` instead of its subclass
    `~errors.AutoReconnect` if the server is unavailable. Applications
    that expect to catch `~errors.AutoReconnect` should now catch
    `~errors.ConnectionFailure` while creating a new connection.
...
- A new `~collection.Collection.aggregate` method to support
...
`~replica_set_connection.ReplicaSetConnection`
to run inline map reduce commands on secondaries. See
`~collection.Collection.inline_map_reduce` for details.
...
- PyMongo supports the new BSON binary subtype 4 for UUIDs. The default
  ...
  `~collection.Collection.uuid_subtype`
  The current default remains `~bson.binary.OLD_UUID_SUBTYPE` but will
  be changed to `~bson.binary.UUID_SUBTYPE` in a future release.
...
- Secondaries can be used for `~cursor.Cursor.count`,
  `~cursor.Cursor.distinct`, `~collection.Collection.group`,
...
- PyMongo now performs automatic per-socket database authentication. You no
  ...
  application calls `~database.Database.logout`.
...
- `~collection.Collection.map_reduce` will accept a `dict` or
...
- If only one host:port pair is specified `~connection.Connection`
...
.. warning::
    ...
    The default subtype for `~bson.binary.Binary` has changed
...
- max_pool_size option: previously pymongo had a hard coded pool size
  ...
  size as a parameter to `~connection.Connection`
...
- Find by metadata in GridFS: You can know specify query fields as
  keyword parameters for `~gridfs.GridFS.get_version` and
  `~gridfs.GridFS.get_last_version`.
- Per-query slave_okay option: slave_okay=True is now a valid keyword
  argument for `~collection.Collection.find` and
  `~collection.Collection.find_one`.

API changes:

- `~database.Database.validate_collection` now returns a
...
.. warning::  The ``pool_size``, ``auto_start_request``, and ``timeout`` parameters
              for `~connection.Connection` have been completely
...
- If a read slave raises `~errors.AutoReconnect`
  `~master_slave_connection.MasterSlaveConnection` will now
  retry the query on each slave until it is successful or all slaves have
  raised `~errors.AutoReconnect`. Any other exception will
  ...
  `~errors.AutoReconnect` was immediately raised in case
...
- added `~collection.Collection.find_and_modify`.
- added `~collection.Collection.inline_map_reduce`.
- changed `~collection.Collection.map_reduce`.
...
.. warning:: MongoDB versions greater than 1.7.4 no longer generate temporary
   ...
   the same name. `~collection.Collection.map_reduce` now
   requires the ``out`` parameter.

Issues resolved
...............

- PYTHON-225: `~objectid.ObjectId` class definition should use __slots__.
- PYTHON-223: Documentation fix.
- PYTHON-220: Documentation fix.
- PYTHON-219: KeyError in `~collection.Collection.find_and_modify`
- PYTHON-213: Query server for maximum BSON document size.
- PYTHON-208: Fix `~connection.Connection` __repr__.
...
- PYTHON-184: Raise `~errors.DuplicateKeyError` for duplicate keys in capped collections.
...
- PYTHON-170: Add `~collection.Collection.find_and_modify`.
...
.. warning::
  ...
  `~errors.PyMongoError`, then the exceptions raised
...
- added `~cursor.Cursor.batch_size`.
...
- added `~connection.Connection.nodes` property.
- added a maximum pool size of 10 sockets.
- added support for replica sets.
- DEPRECATED `~connection.Connection.from_uri` and
  `~connection.Connection.paired`, both are supplanted
  by extended functionality in `~connection.Connection`.
- added tz aware support for datetimes in
  `~bson.objectid.ObjectId`,
  `~bson.timestamp.Timestamp` and `~bson.json_util`
  methods.
- added `~collection.Collection.drop` helper.
- reuse the socket used for finding the master when a
  `~connection.Connection` is first created.
- added support for `~bson.min_key.MinKey`,
  `~bson.max_key.MaxKey` and
  `~bson.timestamp.Timestamp` to `~bson.json_util`.
...
- fixed issue where connection pool was being shared across
  `~connection.Connection` instances.
...
- improvements to the `~database.Database.command` API.
- fixed `~collection.Collection.drop_indexes` behavior
...
------------------------
- added `.GridFile._id` property for
  `.GridFile` instances.
- fix for making a `~connection.Connection` (with
  ``slave_okay`` set) directly to a slave in a replica pair.
- accept kwargs for
  `~collection.Collection.create_index` and
  `~collection.Collection.ensure_index` to support all
  indexing options.
...
- DEPRECATED ``options`` argument to
  ...
  `~database.Database.create_collection` in favor of
  kwargs.
- added `~has_c` to check for C extension.
- added `~connection.Connection.copy_database`.
- added `~cursor.Cursor.alive` to tell when a cursor
...
- added ``name`` argument for
  `~collection.Collection.create_index` and
  `~collection.Collection.ensure_index`.
- fixed connection pooling w/ fork
- `~connection.Connection.paired` takes all kwargs that
  are allowed for `~connection.Connection`.
- `~collection.Collection.insert` returns list for bulk
  inserts of size one.
- fixed handling of `datetime.datetime` instances in
  `~bson.json_util`.
- added `~connection.Connection.from_uri` to support
  MongoDB connection uri scheme.
- fixed chunk number calculation when unaligned in `gridfs`.
- `~database.Database.command` takes a string for simple
  commands.
- added `~database.Database.system_js` helper for
...
- move ``"_id"`` to front only for top-level documents (fixes some
  corner cases).
- `~collection.Collection.update` and
  `~collection.Collection.remove` return the entire
...
- added `~errors.PyMongoError` as base exception class
  for all `~errors`. this changes the exception hierarchy
  somewhat, and is a BREAKING change if you depend on
  `~errors.ConnectionFailure` being a `IOError`
  or `~bson.errors.InvalidBSON` being a `ValueError`,
  for example.
- added `~errors.DuplicateKeyError` for calls to
  `~collection.Collection.insert` or
  `~collection.Collection.update` with ``safe`` set to
  ``True``.
- removed `~thread_util`.
- added `~database.Database.add_user` and
  `~database.Database.remove_user` helpers.
- fix for `~database.Database.authenticate` when using
  non-UTF-8 names or passwords.
- minor fixes for
  `~master_slave_connection.MasterSlaveConnection`.
- clean up all cases where `~errors.ConnectionFailure`
  is raised.
...
----------------------
- DEPRECATED running `~collection.Collection.group` as
  `~database.Database.eval`, also changed default for
  `~collection.Collection.group` to running as a command
...
- DEPRECATED `~connection.Connection.host`,
  `~connection.Connection.port`,
  `~database.Database.connection`,
  `~database.Database.name`,
  `~collection.Collection.database`,
  `~collection.Collection.name` and
  `~collection.Collection.full_name` in favor of
...
- raise `~bson.errors.InvalidId` instead of
  ...
  `~bson.objectid.ObjectId` that contains non-hexadecimal
  characters
- allow `unicode` instances for `~bson.objectid.ObjectId` init

Changes in Version 1.2
----------------------
- ``spec`` parameter for `~collection.Collection.remove` is
...
- added `~collection.Collection.map_reduce` helper - see
  :doc:`example <examples/aggregation>`
- added `~cursor.Cursor.distinct` method on
  `~cursor.Cursor` instances to allow distinct with
  queries
- fix for `~cursor.Cursor.__getitem__` after
  `~cursor.Cursor.skip`
...
- DEPRECATED `~bson.objectid.ObjectId.url_encode` and
  `~bson.objectid.ObjectId.url_decode` in favor of `str`
  and `~bson.objectid.ObjectId`, respectively
...
------------------------
- improvements to `~collection.Collection.insert` speed
  (using C for insert message creation)
- use random number for request_id
- fix some race conditions with `~errors.AutoReconnect`
...
- added `~collection.Collection.distinct`
...
- DEPRECATED `pymongo.cursor.Cursor.__len__` in favor of
  `~cursor.Cursor.count` with ``with_limit_and_skip`` set
...
- added ``__hash__`` for `~bson.dbref.DBRef` and
  `~bson.objectid.ObjectId`
- bulk `~collection.Collection.insert` works with any
  iterable
- fix `~bson.objectid.ObjectId` generation when using
  `multiprocessing`
- added `~cursor.Cursor.collection`
...
- added ``safe`` parameter for `~collection.Collection.remove`
- added ``tailable`` parameter for `~collection.Collection.find`

Changes in Version 1.0
----------------------
- fixes for
  `~master_slave_connection.MasterSlaveConnection`
- added ``finalize`` parameter for `~collection.Collection.group`
- improvements to `~collection.Collection.insert` speed
- improvements to `gridfs` speed
- added `~cursor.Cursor.__getitem__` and
  `~cursor.Cursor.__len__` for
  `~cursor.Cursor` instances

Changes in Version 0.16
-----------------------
- support for encoding/decoding `uuid.UUID` instances
- fix for `~cursor.Cursor.explain` with limits
...
- API CHANGE no longer need to specify direction for
  `~collection.Collection.create_index` and
  `~collection.Collection.ensure_index` when indexing a
  single key
...
-------------------------
- `.GridFile.seek` and
  `.GridFile.tell` for (read mode)
  `.GridFile` instances

Changes in Version 0.14
-----------------------
- support for long in `~bson.BSON`
- added `~collection.Collection.rename`
...
- better
  `~master_slave_connection.MasterSlaveConnection`
  support
- API CHANGE `~collection.Collection.insert` and
  `~collection.Collection.save` both return inserted
  ``_id``
...
- added `~errors.AutoReconnect` exception for when
  reconnection is possible
- make `gridfs` thread-safe
- fix for `gridfs` with non `~bson.objectid.ObjectId` ``_id``
...
-------------------------
- PEP 8
- updates for `~collection.Collection.group`
- VS build
...
- added ``timeout`` parameter for `~connection.Connection`

Changes in Version 0.10.3
-------------------------
- fix bug with large `~cursor.Cursor.limit`
...
-------------------------
- alias `~connection.Connection` as
  `pymongo.Connection`
- raise an exception rather than silently overflowing in encoder

Changes in Version 0.10
-----------------------
- added `~collection.Collection.ensure_index`
