...
To get a `~pymongo.database.Database` instance from a
`MongoClient` use either dictionary-style or attribute-style
access:
...
class MongoClient(common.BaseObject):
    ...
    def __init__(
        ...
        `~pymongo.errors.ConnectionFailure` is raised and the client
        reconnects in the background. Application code should handle this
        exception (recognizing that the operation failed) and then continue to
        execute.

        The ``port`` parameter can be a full `mongodb URI
        ...
        :Parameters:
          ...
          - ``tz_aware`` (optional): if ``True``,
            `~datetime.datetime` instances returned as values
          ...
          - ``read_preference``: The read preference for this client. If
            ...
            *other* than PRIMARY is required - otherwise all queries will throw
            ...
            See `~pymongo.read_preferences.ReadPreference` for all
        ...
        .. versionchanged:: 3.0
           ...
           `~.mongo_replica_set_client.MongoReplicaSetClient`: it can connect
           ...
           longer raises `~pymongo.errors.ConnectionFailure` if they
           are unavailable, nor `~pymongo.errors.ConfigurationError`
           ...
           In PyMongo 2.x, `MongoClient` accepted a list of
           ...
           The ``MongoClient.disconnect`` method is removed; it was a
           synonym for `close`.

           `~.mongo_client.MongoClient` no longer returns an
           instance of `~pymongo.database.Database` for attribute names
    ...
    def host(self):
        ...
        .. warning:: An application that accesses `.host` and `.port`
           is vulnerable to a race condition, if the client switches to a
           new primary or mongos in between. Use `.address` instead.
        """
    ...
    def max_pool_size(self):
        ...
        a blocked operation will raise `~pymongo.errors.ConnectionFailure`
    ...
    def set_cursor_manager(self, manager_class):
        ...
        Raises `TypeError` if ``manager_class`` is not a subclass of
        `~pymongo.cursor_manager.CursorManager`. A cursor manager
    ...
    def _get_topology(self):
        """Get the internal `~pymongo.topology.Topology` object.
    ...
    def drop_database(self, name_or_database):
        ...
        :Parameters:
          - ``name_or_database``: the name of a database to drop, or a
            `~pymongo.database.Database` instance representing the
            database to drop
        """
    ...
    def get_database(self, name, codec_options=None,
        ...
        """Get a `~pymongo.database.Database` with the given name and
        options.
